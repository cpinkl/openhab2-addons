...
void JNICALL Java_org_openhab_binding_hideki_internal_HidekiDecoder_setTimeOut(JNIEnv* env, jobject object, jint timeout)
{
...
void JNICALL Java_org_openhab_binding_hideki_internal_HidekiDecoder_create(JNIEnv* env, jobject object, jint receiver, jint pin)
{
  env->MonitorEnter(object);
  jmethodID getId = env->GetMethodID(env->GetObjectClass(object), "getId", "()I");
  if (getId != nullptr) {
...
//    syslog(LOG_INFO, "Create Decoder: %X, Id %d, with receiver %d\n", object, id, receiver);
    ...
    if ((DecoderMap.find(id) == DecoderMap.end()) && (it != ReceiverMap.end())) {
      Decoder* decoder = new Decoder(pin, *(it->second));
